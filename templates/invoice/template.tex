...
$if(seriffont)$\setromanfont [Ligatures={Common}, Numbers={OldStyle}]{$seriffont$}$endif$
$if(sansfont)$\setsansfont[Scale=0.9]{$sansfont$}$endif$
...
\small
$if(name)$
    % \textsc{\textbf{$name$$if(surname)$ $surname$$endif$}}
    \textbf{$name$$if(surname)$ $surname$$endif$}
$endif$
$if(from)$
$for(from)$
...
$endfor$
$else$
$if(address)$\textbullet{} $address$$endif$
$if(city)$\textbullet{} $if(zip)$$zip$$endif$ $city$$endif$
$endif$
...
% \section*{\textsc{Invoice} \textsc{\#$date$-$invoice-nr$}}
\section*{\textbf{Invoice} \textbf{\#$date$-$invoice-nr$}}
\footnotesize
\newcounter{pos}
\setcounter{pos}{0}
...
\begin{itemize}
    \item[] \textit{$company.account.name$} \\
            Bank: $company.account.bank$ \\
            IBAN: $company.account.iban$ \\
            BIC: $company.account.bic$ \\
\end{itemize}
