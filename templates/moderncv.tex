...
% moderncv themes
$if(moderncvstyle)$
\moderncvstyle[]{$moderncvstyle$}                     % style options are 'casual' (default), 'classic', 'banking', 'oldstyle', 'fancy' and 'contemporary'
$else$
...
$if(address)$
\address{$address$}{$if(city)$$city$$endif$}{$if(country)$$country$$endif$}
$endif$
...
$if(researchgate)$
\social[researchgate]{$researchgate$}
$endif$

$if(researcherid)$
\social[researcherid]{$researcherid$}
$endif$

$if(googlescholar)$
\social[googlescholar]{$googlescholar$}
$endif$
...
$if(signal)$
\social[signal]{$signal$}
$endif$

$if(matrix)$
\social[matrix]{$matrix$}
$endif$
...
$if(inspire)$
\social[inspire]{$inspire$}
$endif$


\extrainfo{additional information}                 % optional, remove / comment the line if not wanted
\photo[64pt][2pt]{$if(picture)$$picture$$else$picture$endif$}                       % optional, remove / comment the line if not wanted; '64pt' is the height the picture must be resized to, 2pt is the thickness of the frame around it (put it to 0pt for no frame) and 'picture' is the name of the picture file
