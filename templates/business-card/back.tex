...
% define colours
\definecolor{bordercolour}{HTML}{4F77C1}
...
\begin{document}
  ...
  \begin{tikzpicture}[remember picture,overlay,inner sep=0pt]
    \draw[fill=bordercolour,draw=bordercolour!30] (current page.center) ++(-\paperwidth/2+\edgemargin,\paperheight/2-\edgemargin) rectangle ++(\paperwidth-2*\edgemargin,-\paperheight+2*\edgemargin);
    % logo
$if(logo)$
    \draw (current page.center) ++(0,\paperheight/2-\edgemargin-\paperwidth/2+\edgemargin+\logowidth/2) node (helper logo) {};
    \node[anchor=north] at (helper logo) {\includegraphics[width=\logowidth]{$logo$}};
$endif$
    % qr code
...
VERSION:3.0
N:$if(name)$$name$$endif$;$if(surname)$$surname$$endif$;;.
FN:$if(name)$$name$$endif$ $if(surname)$$surname$$endif$
TITLE:$if(position)$$position$$endif$
ORG:$if(organization)$$organization$$endif$
% PHOTO;VALUE=URI;TYPE=JPEG:https://johndoe.com/path/to/jpeg/images.jpeg
TEL;TYPE=MOBILE:$if(mobile)$$mobile$$endif$
EMAIL:$if(email)$$email$$endif$
URL:$if(www)$$www$$endif$
REV:$if(date)$$date$$endif$T13:52:43Z
BDAY:19880310
      ADR;TYPE=WORK,PREF:;;$if(address)$$address$$endif$;$if(city)$$city$$endif$;SF;$if(zip)$$zip$$endif$;USA
END:VCARD}};
  \end{tikzpicture}
  \vspace*{\fill}
\end{document}
